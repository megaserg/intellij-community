...
public class DownloadCacheAction extends AbstractCacheAction {
  ...
  private boolean compareAndUpdate(String oldStorageFilesPrefix,
    ...
    if (ProjectHashUtil.compareAndUpdate(myTempArchiveDirectory, oldStorageFilesPrefix,
                                         myTempArchiveDirectory, newStorageFilesPrefix,
                                         zipFilePath, actualDirectoryFile)) {
  ...
  private void downloadAndDecompress(@Nullable Project project, ProgressIndicator indicator) {
    ...
    try {
      ...
      downloadFile(ftp, OUTPUT_ZIP_NAME, myLocalOutputZipPath);
      downloadFile(ftp, OUTPUT_HASHES_FILE_NAME, myLocalOutputHashesPath);
      downloadFile(ftp, OUTPUT_TREE_FILE_NAME, myLocalOutputTreePath);
